...
func Init() {
	...
	pflag.StringP("label-file", "f", "labels.yaml", "Map labels config file to dynamic label in Prometheus metrics")
	...
	logrus.Debug(err)
	err = viper.BindEnv("label-file")
	logrus.Debug(err)

	if viper.IsSet("label-file") {
		file, err := os.Open(viper.GetString("label-file"))
		if err != nil {
			logrus.Panic(err)
		}
